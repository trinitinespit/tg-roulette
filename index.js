...
app.post("/tg-webhook", async (req, res) => {
  ...
  if (message?.text === "/start" || message?.text?.startsWith("/start ")) {
    ...
    const GREETINGS = {
      ...
      de: {
        text: (name) => `👋 Hallo, ${name}!\n\n🎲 *Spinny* ist ein zufälliger Videochat direkt in Telegram.\n\nTippe unten auf den Button, erlaube den Kamerazugriff — und in Sekunden bist du mit jemand Neuem aus aller Welt verbunden.\n\n• 🌍 Matching nach Sprache\n• ❤️ Reaktionen während des Anrufs\n• ⭐ Premium mit Geschlechterfilter\n• 🚩 Moderationssystem`,
        button: "🎲 Spinny öffnen",
      },
      tr: {
        text: (name) => `👋 Merhaba, ${name}!\n\n🎲 *Spinny*, doğrudan Telegram içinde rastgele bir video sohbettir.\n\nAşağıdaki butona dokun, kamera erişimine izin ver — ve saniyeler içinde dünyanın herhangi bir yerinden yeni biriyle bağlantı kur.\n\n• 🌍 Dile göre eşleştirme\n• ❤️ Görüşme sırasında tepkiler\n• ⭐ Cinsiyet filtreli Premium\n• 🚩 Moderasyon sistemi`,
        button: "🎲 Spinny'yi Aç",
      },
      es: {
...
<div class="content">
  ...
  <div class="section" id="tab-users">
    ...
    <div style="margin-top:12px;text-align:center;">
      <button class="btn" id="loadMoreUsersBtn" onclick="loadMoreUsers()">Загрузить ещё 100</button>
    </div>
  </div>
...
async function loadMoreUsers() {
  ...
  try {
    ...
    if (!data.hasMore) {
      btn.textContent = 'Больше пользователей нет';
      btn.disabled = true;
    } else {
      btn.textContent = 'Загрузить ещё 100';
      btn.disabled = false;
    }
  } catch (e) {
...
app.get("/admin/users-page", adminAuth, async (req, res) => {
  ...
  try {
    ...
    console.error("[admin] ошибка постраничной загрузки:", e.message);
